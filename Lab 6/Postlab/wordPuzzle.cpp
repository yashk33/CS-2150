...
int main(int argc, char **argv){
  ...
  //getting desired size of hashTable
  ...
  hashTable *mainTable = new hashTable(size); //declaration of hashTable

  file.open(dictionary.c_str());

  int maxWordLength = 0; //used for optimization
  ...
  list<string> *ret = new list<string>; //will store return values
  int numberFound = 0; //will store how many words are found

  timer t;
  t.start();

  for(int r = 0; r < rowCount; r++){ //loop to traverse through rows
    
    for(int c = 0; c < colCount; c++){ //loop to traverse through columns
      
      for(int direction = 0; direction < 8; direction++){ //loop to traverse through direction
	
	for(int s = 3; s <= maxWordLength; s++){ //loop to search length of word
	  
	  string getWord = getWordInGrid(r, c, direction, s, rowCount, colCount); //gets the word in grid

	  //checks if it is an actual word
	   if(mainTable->contains(getWord)){
	    ...
	    string temp = dir + "(" + to_string(r) + ", " + to_string(c) + "): " + getWord; //stores word in correct format
	    
	    //adds to our return list and increments numberFound
